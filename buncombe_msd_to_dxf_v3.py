...
APP_NAME = "Buncombe MSD to DXF v3"
...
def line_label(name, p):
    ...
    if name == "Gravity Mains":
        ...
        to = val(p, "TOMH")
        if frm or to:
            lines.append("MH " + " - ".join(str(x) for x in [frm, to] if x))
        ...
        if elev:
            lines.append("  ".join(elev))
        return "\n".join(lines)

    if name == "Laterals":
        st = val(p, "SERVICETYPE")
        return " ".join(x for x in [dia, mat, st, "SEWER LATERAL"] if x)
...
def mh_label(p):
    ...
    if ident:
        lines.append(f"MH {ident}")

    rim = fmt_num(val(p, "RIMELEV"))
    if rim:
        lines.append(f"RIM {rim}")

    invout = fmt_num(val(p, "INVERTELEV"))
    if invout:
        lines.append(f"INV OUT {invout}")

    for key, lab in [
        ("INVERT", "INV IN 1"),
        ("INVERTTWO", "INV IN 2"),
        ("INVERTTHREE", "INV IN 3"),
        ("INVERTFOUR", "INV IN 4"),
        ("DROPINV", "DROP INV"),
    ]:
        v = fmt_num(val(p, key))
...
def add_mtext(msp, text, xy, layer, height=5.0, rotation=0.0):
    if not text:
        return
    e = msp.add_mtext(
        text.replace("\n", "\\P"),
        dxfattribs={"layer": layer, "char_height": height, "rotation": rotation}
    )
    e.set_location(xy)
...
def make_layers(doc):
    ...
    for name, color in defs.items():
        if name not in doc.layers:
            doc.layers.add(name, color=color)

def add_line_feature(msp, doc, name, cfg, geom, props, label_all):
    xd = all_xdata(props)
    ...
    for part in parts:
        ...
        # IMPORTANT v3: preserve the exact source geometry from MSD.
        # No clipping, resampling, reconstruction, simplification, or segmentation.
        ...
        if label_all:
            x, y, ang = line_midpoint_angle(pts)
            add_mtext(msp, line_label(name, props), (x, y + 5.0), cfg["label"], 5.0, ang)

        count += 1
    return count

def add_point_feature(msp, name, cfg, geom, props, label_all):
    xd = all_xdata(props)
    gtype = geom.get("type")
    coords = geom.get("coordinates")
    ...
    for p in points:
        ...
        ent = msp.add_circle((x, y), radius=2.0, dxfattribs={"layer": cfg["cad"]})
        if xd:
            ent.set_xdata("MSDATTR", xd)

        if label_all:
            add_mtext(msp, mh_label(props), (x + 6.0, y + 6.0), cfg["label"], 5.0, 0.0)
...
def create_dxf(cx, cy, buffer_ft, selected, out, label_all, logger):
    ...
    for name in selected:
        ...
        for ft in gj.get("features", []):
            ...
            if cfg["kind"] == "line":
                n += add_line_feature(msp, doc, name, cfg, geom, props, label_all)
            else:
                n += add_point_feature(msp, name, cfg, geom, props, label_all)
    ...
    note = (
        ...
        f"Retrieved {datetime.now():%Y-%m-%d %H:%M} - FIELD VERIFY UTILITIES."
    )
    add_mtext(msp, note, (xmin, ymin - 20.0), "MSD-GIS-NOTE", 5.0)
...
class App(tk.Tk):
    ...
    def build(self):
        ...
        ttk.Label(self, text="Buncombe MSD → DXF v3", font=("Segoe UI", 18, "bold")).pack(
            anchor="w", padx=16, pady=(16,8)
        )

        f = ttk.LabelFrame(self, text="Project Location")
        ...
        ttk.Label(f, text="X").grid(row=1, column=1)
        ttk.Entry(f, textvariable=self.x, width=16).grid(row=1, column=2, **p)
        ttk.Label(f, text="Y").grid(row=1, column=3)
        ttk.Entry(f, textvariable=self.y, width=16).grid(row=1, column=4, **p)

        ttk.Label(f, text="Buffer (ft)").grid(row=2, column=0, sticky="w", **p)
        ttk.Entry(f, textvariable=self.buffer, width=12).grid(row=2, column=1, sticky="w", **p)

        l = ttk.LabelFrame(self, text="MSD Data")
        l.pack(fill="x", padx=16, pady=6)
        for i, (n, v) in enumerate(self.vars.items()):
            ttk.Checkbutton(l, text=n, variable=v).grid(row=0, column=i, sticky="w", **p)
        ttk.Checkbutton(l, text="Create full GIS labels", variable=self.labels).grid(
            row=1, column=0, columnspan=3, sticky="w", **p
        )

        o = ttk.LabelFrame(self, text="DXF Output")
        o.pack(fill="x", padx=16, pady=6)
        ttk.Entry(o, textvariable=self.output).grid(row=0, column=0, sticky="ew", **p)
        ttk.Button(o, text="Browse / Save As", command=self.browse).grid(row=0, column=1, **p)
    ...
    def browse(self):
        p = filedialog.asksaveasfilename(
            ...
            initialfile="Buncombe_MSD_Export.dxf",
            filetypes=[("DXF", "*.dxf")]
        )
        if p:
            self.output.set(p)
    ...
    def start(self):
        ...
        if not sel:
            messagebox.showerror(APP_NAME, "Select at least one layer.")
            return

        try:
            b = float(self.buffer.get())
        except:
            messagebox.showerror(APP_NAME, "Buffer must be numeric.")
            return
